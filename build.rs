...
fn build_librdkafka() {
    ...
    // Enable Snappy compression support (built-in to librdkafka)
    // The cmake build does not set WITH_SNAPPY by default, even though
    // librdkafka has a built-in snappy implementation. We must explicitly
    // enable it to decompress snappy-compressed Kafka messages.
